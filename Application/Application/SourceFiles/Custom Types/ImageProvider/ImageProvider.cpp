...
QImage ImageProvider::requestImage(const QString &id, QSize *size, const QSize &requestedSize)
{
	...
	if (m_list.find(args[1]) != m_list.end())
	{
		if (args[1] == "default")
		{
			return m_defaultImage.scaled(res, res, Qt::KeepAspectRatio, Qt::SmoothTransformation);
		}
		return m_list.value(args[1]).second.scaled(res, res, Qt::KeepAspectRatio, Qt::SmoothTransformation);
	}
	return QImage();
}
...
QString ImageProvider::AppendImage(QObject* const object, QImage image)
{
	...
	QList<QObject*> t;
	t.append(object);
	m_list.insert(QString::number(m_safeId), QPair<QList<QObject*>, QImage>(t, image));
